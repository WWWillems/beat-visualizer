...
void main() {
  float band = aSeed.x;
  float theta = aSeed.y * 6.28318530718 + uTime * uSpin * (0.22 + band);
  float rough = sin(theta * (3.0 + band * 9.0) + aSeed.w * 6.28318530718 + uTime * 0.8);
  float radius = uRadius + (band - 0.5) * uThickness + aLevel * uBurst * 0.55 + rough * uThickness * 0.12;
  vec2 dir = vec2(cos(theta), sin(theta));
  vec3 pos = vec3(
    dir * radius,
    (aLevel - 0.5) * uDepth * 0.9 + sin(theta * 2.0 + uTime) * uDepth * 0.16
  );

  vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
  gl_Position = projectionMatrix * mvPosition;
  gl_PointSize = uSize * uPixelRatio * (2.2 / max(0.1, -mvPosition.z));
  vAlpha = (0.18 + aLevel * 0.82) * (0.35 + aSeed.z * 0.65);
...
void main() {
  ...
  gl_FragColor = vec4(vec3(1.0), core * vAlpha * vDepth * uBrightness * uBrightness * uEnergy);
}
`;
...
export class SpectralHaloInstance {
  ...
  setSize(width: number, height: number): void {
    configureDepthCamera(this.camera, width / height, 0, 0.55, 0);
  ...
  render(
    ...
    for (let i = 0; i < this.levels.length; i++) {
      const bin = this.bandIndices[i];
      this.levels[i] = this.spectrumFrame[bin] ?? 0;
    }
    ...
    this.material.uniforms.uRadius.value = resolveParam(clip, "radius", timelineTime, features) * (0.95 + gates.motion * 0.16);
    this.material.uniforms.uThickness.value = resolveParam(clip, "thickness", timelineTime, features);
    this.material.uniforms.uSpin.value = resolveParam(clip, "spin", timelineTime, features) * (0.8 + gates.motion * 0.5);
    this.material.uniforms.uBurst.value = resolveParam(clip, "burst", timelineTime, features) + gates.accent * 0.28;
    ...
    const countNorm = Math.min(1, 1800 / Math.max(1, count));
